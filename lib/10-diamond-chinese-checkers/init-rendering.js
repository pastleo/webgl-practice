...
export default function initRendering(game) {
  ...
  bufferVaos.cone = createInstancedBufferVao(gl, programs.main,
    twgl.primitives.createTruncatedConeVertices(1, 0, 2, 32, 32),
    {
      ...
      ai_diffuse: {
        ...
        data: p => pieceTeamDiffuseMap[p.team],
      },
  ...
  bufferVaos.torus = createInstancedBufferVao(gl, programs.main,
    ...
    {
      ...instanceWorldMatriceAttrs(
        l => matrix4.translation(...pieceCoordTranslation(l.coord)),
      ),
    ...
    twgl.primitives.createXYQuadVertices(),
    {
      ...instanceWorldMatriceAttrs(
        (_, i) => pipe(
          ...
          m => matrix4.translate(m, 0, 0.05, -20),
...
function createInstancedBufferVao(gl, program, vertices, instanceAttrs, instanceData, instanceDrawCount) {
  ...
  Object.entries(instanceAttrs).forEach(([attr, attrContent]) => {
    ...
    gl.bufferData(
      gl.ARRAY_BUFFER,
      flatMapOrFillFloat32Array(instanceData, mapperFnOrData),
      gl.DYNAMIC_DRAW,
    );
  ...
  bufferInfo.instanceDrawCount = instanceDrawCount !== undefined ? instanceDrawCount : instanceData.length;
...
export function createMirrorInfo(gl) {
  const size = gl.canvas.clientWidth >= 1600 ? 2048 : 1024;
  const framebufferInfo = twgl.createFramebufferInfo(gl, null, size, size);
