...
vec3 calcCubeDir(vec3 coord) {
  ...
  vec3 color = sin(coord * u_time) * 0.5 + 0.5;
  return vec3(saturatedNoise) * color * u_starColor;
}

vec3 calcLayer(vec3 cubeCoord) {
  vec3 color = vec3(0);

  color += smoothstep(SIN30, SIN45, cubeCoord.x) * calcCubeDir(vec3(1,0,0) + cubeCoord * vec3(0,1,1));
  color += smoothstep(SIN30, SIN45, -cubeCoord.x) * calcCubeDir(vec3(-1,0,0) + cubeCoord * vec3(0,1,1));
  color += smoothstep(SIN45, SIN60, cubeCoord.y) * calcCubeDir(vec3(0,1,0) + cubeCoord * vec3(1,0,1));
  color += smoothstep(SIN45, SIN60, -cubeCoord.y) * calcCubeDir(vec3(0,-1,0) + cubeCoord * vec3(1,0,1));
  color += smoothstep(SIN30, SIN45, cubeCoord.z) * calcCubeDir(vec3(0,0,1) + cubeCoord * vec3(1,1,0));
  color += smoothstep(SIN30, SIN45, -cubeCoord.z) * calcCubeDir(vec3(0,0,-1) + cubeCoord * vec3(1,1,0));
...
void main() {
  ...
  color += calcLayer(rotationZ(0.0) * rotationY(0.0) * cubeCoord);
  color += calcLayer(rotationY(-DEG45) * rotationX(-DEG45) * cubeCoord);
  color += calcLayer(rotationY(-DEG45 * 5.0) * rotationX(-DEG45) * cubeCoord);
