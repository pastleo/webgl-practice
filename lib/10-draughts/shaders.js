...
void main() {
  mat4 worldMatrix = u_use_ai_world ? ai_world * u_world : u_world;
...
vec3 calcCubeDir(vec2 uv, float rndSeed) {
  uv *= 64.86;
  vec3 color = vec3(0);
  ...
  for(int i = 0; i <= 1; i++) {
    for(int j = 0; j <= 1; j++) {
      ...
      float rnd = hash21(subId + rndSeed + u_seed);
...
void main() {
  ...
  vec3 asinCoord = asin(cubeCoord) * 2.0 / PI;

  color += smoothstep(SIN30, SIN45, cubeCoord.x) * calcCubeDir(asinCoord.yz, 12.34);
  color += smoothstep(SIN30, SIN45, -cubeCoord.x) * calcCubeDir(asinCoord.yz, 23.58);
  color += smoothstep(SIN45, SIN60, cubeCoord.y) * calcCubeDir(asinCoord.xz, 84.92);
  color += smoothstep(SIN45, SIN60, -cubeCoord.y) * calcCubeDir(asinCoord.xz, 73.75);
  color += smoothstep(SIN30, SIN45, cubeCoord.z) * calcCubeDir(asinCoord.xy, 94.48);
  color += smoothstep(SIN30, SIN45, -cubeCoord.z) * calcCubeDir(asinCoord.xy, 87.76);
