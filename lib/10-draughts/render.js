...
export default function render(game, time) {
  ...
  renderSkybox(gl, game.rendering, projectionMatrix, viewMatrix);
}

export function renderObjects(gl, rendering, programInfo, enableBlend) {
  drawCones(gl, rendering, programInfo);
  drawAvailableCoords(gl, rendering, programInfo, enableBlend);
}

function renderBackground(gl, rendering, programInfo) {
  drawLocations(gl, rendering, programInfo);
  drawGround(gl, rendering, programInfo);
  drawKanban(gl, rendering, programInfo);
}
...
function drawAvailableCoords(gl, rendering, programInfo, enableBlend) {
  const worldMatrix = pipe(
    matrix4.identity(),
...
function drawGround(gl, rendering, programInfo) {
  const worldMatrix = pipe(
    matrix4.identity(),
...
function drawKanban(gl, rendering, programInfo) {
  const worldMatrix = pipe(
    ...
    m => matrix4.translate(m, 0, 0.01, -20),
  ...
  gl.bindVertexArray(rendering.bufferVaos.kanban.vao);
  drawElementsInstanced(gl, rendering, 'kanban');
...
function renderSkybox(gl, rendering, projectionMatrix, viewMatrix) {
  const programInfo = rendering.programs.skybox;
  gl.useProgram(programInfo.program);
  twgl.setUniforms(programInfo, {
    u_time: Date.now(),
    u_viewInvere: matrix4.inverse(
      matrix4.multiply(
        projectionMatrix,
        [
  ...
  const { bufferInfo, vao } = rendering.bufferVaos.skyboxXyQuad;
