...
export default function render(game, time) {
  ...
  drawLocations(gl, game.rendering, game.rendering.programs.main);
  ...
  renderGround(gl, game);

  gl.useProgram(game.rendering.programs.main.program);

  drawAvailableCoords(gl, game.rendering, game.rendering.programs.main);
  drawKanban(gl, game.rendering, game.rendering.programs.main);
...
function drawGround(gl, rendering, programInfo) {
  const worldMatrix = pipe(
    matrix4.identity(),
    m => matrix4.scale(m, 50, 1, 50),
    m => matrix4.xRotate(m, degToRad(-90)),
  );

  twgl.setUniforms(programInfo, {
    u_use_ai_world: false,
    u_world: worldMatrix,
    u_worldInverseTranspose: matrix4.transpose(matrix4.inverse(worldMatrix)),
    u_diffuse: [26/255, 28/255, 53/255, 1],
    u_ambient: [0, 0, 0],
    u_emission: [0, 0, 0],
    u_specular: [1, 1, 1],
    u_shininess: 20000,
...
// if eventually not using its own program, tear down this function
function renderGround(gl, game) {
  const rendering = game.rendering;
  const programInfo = rendering.programs.main;
